...
export async function getTestimonialsByProperty(propertySlug: string): Promise<Testimonial[]> {
  try {
    const response = await cosmic.objects
      .find({
        type: 'testimonials',
        'metadata.property': propertySlug
      })
      .props(['id', 'title', 'slug', 'metadata'])
      .depth(1)
    return response.objects as Testimonial[]
...
export async function getPropertiesByHost(hostSlug: string): Promise<Property[]> {
  try {
    const response = await cosmic.objects
      .find({
        type: 'properties',
        'metadata.host': hostSlug
      })
      .props(['id', 'title', 'slug', 'metadata'])
      .depth(1)
    return response.objects as Property[]
